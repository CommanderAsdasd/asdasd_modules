...
struct MentalCartesian : Module {
  ...
  //   NUM_PARAMS = KNOB_PARAM + 16		
    ...
    Y_RESET,
		NUM_INPUTS,
    TRIG_STEP_INPUT = BUTTON_PARAM + 16
	};
	enum OutputIds {
		UNQUANT_OUT,
    QUANT_OUT,
...
void MentalCartesian::step() {
    ...
    for (int i = 0 ; i < 4 ; i++)
    {
      for (int j = 0 ; j < 4 ; j++)
      {
        if ((inputs[BUTTON_PARAM + i + j * 4].value)) {
        // if ((inputs[TRIG_STEP_INPUT + i + j * 4].value) > 0.0)
        // {
          // printf("%f \n", inputs[TRIG_STEP_INPUT + i + j * 4].value);
...
MentalCartesianWidget::MentalCartesianWidget(MentalCartesian *module) : ModuleWidget(module)
{
  ...
  for ( int i = 0 ; i < 4 ; i++)
  {
    for ( int j = 0 ; j < 4 ; j++)
    {
      ...
      // addParam(ParamWidget::create<GateInPort>(Vec(button_offset+left+column_spacing * i, top + row_spacing * j + 150 ), module, MentalCartesian::TRIG_STEP_INPUT + i + j * 4, 0.0, 1.0, 0.0));
      addInput(Port::create<GateInPort>(Vec(button_offset+left+column_spacing * i, top + row_spacing * j + 150 ), Port::INPUT, module, MentalCartesian::BUTTON_PARAM + i + j * 4));
